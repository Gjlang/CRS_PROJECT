...
public class AuthFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        // Allow public
        if (PUBLIC_PATHS.contains(path) || path.startsWith("/assets/") || path.startsWith("/css/") || path.startsWith("/js/")) {
        ...
        // RBAC enforcement
        if (path.startsWith("/admin/") && !"COURSE_ADMIN".equals(role)) {
            resp.sendError(HttpServletResponse.SC_FORBIDDEN, "COURSE_ADMIN role required.");
            return;
        }
        if (path.startsWith("/academic/") && !"ACADEMIC_OFFICER".equals(role)) {
