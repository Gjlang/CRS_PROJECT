...
public class UserManagementEJB {
    ...
    public long createUser(User u, String plainPassword) throws SQLException {
        ...
        if (u.getRole() == null) u.setRole("ACADEMIC_OFFICER");
    ...
    public void updateUser(User u) throws SQLException {
        validateUser(u);
        if (u.getUserId() <= 0) throw new IllegalArgumentException("Invalid userId.");
        u.setEmail(u.getEmail().trim().toLowerCase());
        UserDAO dao = new UserDAO();
        dao.update(u);
    }
    ...
    public List<User> listUsers() throws SQLException {
        ...
        for (User u : list) u.setPasswordHash(null);
        return list;
    }
    ...
    public void resetPassword(long userId, String plainPassword) throws SQLException {
        if (userId <= 0) throw new IllegalArgumentException("Invalid userId.");
        if (plainPassword == null || plainPassword.length() < 6) throw new IllegalArgumentException("Password must be at least 6 characters.");
